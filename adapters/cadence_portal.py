...
class CadencePortalAdapter(BaseJobAdapter):
    @property
    def portal_id(self) -> str:
        return "cadence_careers"

    @property
    def portal_name(self) -> str:
        return "Cadence Careers Portal"

    async def scrape(self) -> List[JobListing]:
        # Base URL to establish cookies, CORS context and session parameters
        base_url = "https://cadence.wd1.myworkdayjobs.com/External_Careers"
        logger.info(f"Navigating to Cadence Careers page to establish session context: {base_url}")
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                while offset < max_safety_limit:
                    logger.info(f"Fetching Cadence careers listings starting at offset {offset}...")
                    
                    # Prepare POST request payload with exact filters
                    payload = {
                        "appliedFacets": {
                            "Location_Country": ["c4f78be1a8f14da0ab49ce1162348a5e"],
                            "Location_Region_State_Province": ["c125e15453b44c968e77a8f2516b113f"]
                        },
                        "limit": limit,
                        "offset": offset,
                        "searchText": ""
                    ...
                    result = await page.evaluate(f"""
                        async () => {{
                            try {{
                                const response = await fetch('/wday/cxs/cadence/External_Careers/jobs', {{
                                    method: 'POST',
                                    headers: {{
                    ...
                    if not result:
                        logger.warning(f"Cadence Careers API call at offset {offset} returned empty result.")
                        break
                        
                    if "error" in result and result["error"]:
                        logger.error(f"Error fetching from Cadence API at offset {offset}: {result['error']}")
                        break
                        
                    job_postings = result.get("jobPostings", [])
                    if not job_postings or len(job_postings) == 0:
                        logger.info(f"No more jobs found in Cadence response at offset {offset}. Stopping pagination.")
                        break
                        
                    logger.info(f"Retrieved {len(job_postings)} job postings from Cadence Careers at offset {offset}.")
                    ...
                    for job in job_postings:
                        ...
                        if ext_path.startswith("http"):
                            ...
                            job_listing_link = f"https://cadence.wd1.myworkdayjobs.com/External_Careers{ext_path}"
                ...
                logger.error(f"Failed to scrape Cadence Careers Portal: {e}", exc_info=True)
                raise
            finally:
                await browser.close()
                
        logger.info(f"Finished Cadence Careers scrape. Found total {len(listings)} listings.")
        return listings
